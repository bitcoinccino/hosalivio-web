class PatientChatsController < ApplicationController
  ...
  def show
    ActsAsTenant.with_tenant(@agency) do
      ...
      # without re-running the whole chat-feed load.
  ...
  # Live right-rail refresh. The chart's ActionCable channel sends a
  # "context_changed" nudge when any clinical fact changes; the browser
  # re-fetches this and swaps #patient-clinical-context in place. Rendered
  # per viewer so family gets the family rail and clinicians the clinical
  # one — the same role scoping show uses (no PHI leak via broadcast).
  def clinical_context
    ActsAsTenant.with_tenant(@agency) do
      load_clinical_context
      render partial: "patient_chats/clinical_context", layout: false
    end
  end
  ...
  # Loads every ivar the right-rail clinical-context partial reads. Shared
  # by #show (full page) and #clinical_context (live re-render). Call inside
  # an ActsAsTenant.with_tenant(@agency) block.
